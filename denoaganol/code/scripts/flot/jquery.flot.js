...
(function (B) {
  ...
  B.color.make = function (F, E, C, D) {
    ...
    G.toString = function () {
      if (G.a >= 1) return "rgb(" + [G.r, G.g, G.b].join(",") + ")";
      else return "rgba(" + [G.r, G.g, G.b, G.a].join(",") + ")";
    };
  ...
  B.color.parse = function (F) {
    ...
    if (
      E = /rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)/
        .exec(F)
    ) {
    ...
    if (
      E = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(F)
    ) {
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    function parseOptions(opts) {
      ...
      if (options.grid.tickColor == null) {
        options.grid.tickColor = $.color.parse(options.grid.color).scale(
          "a",
          0.22,
        ).toString();
      }
      ...
      function makeDummyDiv(labels, width) {
        return $(
          '<div style="position:absolute;top:-10000px;' + width +
            'font-size:smaller">' +
            '<div class="' + axis.direction + "Axis " + axis.direction +
            axis.n + 'Axis">' +
            labels.join("") + "</div></div>",
        )
    ...
    function setupGrid() {
      ...
      plotOffset.left = plotOffset.right = plotOffset.top = plotOffset.bottom =
        0;
    ...
    function setupTickGeneration(axis) {
      ...
      // some data points that seemed reasonable
        ...
        noTicks = 0.3 *
          Math.sqrt(axis.direction == "x" ? canvasWidth : canvasHeight);
      }
      ...
      if (opts.mode == "time") {
        ...
        for (var i = 0; i < spec.length - 1; ++i) {
          if (
            delta < (spec[i][0] * timeUnitSize[spec[i][1]] +
      ...
      if (opts.alignTicksWithAxis != null) {
        var otherAxis = (axis.direction == "x"
          ? xaxes
    ...
    function drawGrid() {
      ...
      if (markings) {
        ...
        for (i = 0; i < markings.length; ++i) {
          ...
          // clip
          if (
            xrange.to < xrange.axis.min || xrange.from > xrange.axis.max ||
    ...
    function insertAxisLabels() {
      ...
      for (var j = 0; j < axes.length; ++j) {
        ...
        for (var i = 0; i < axis.ticks.length; ++i) {
          ...
          html.push(
            '<div class="tickLabel" style="' + style.join(";") + '">' +
              tick.label + "</div>",
          );
        }
        html.push("</div>");
    ...
    function drawSeriesPoints(series) {
      function plotPoints(
        ...
        for (var i = 0; i < points.length; i += ps) {
          ...
          if (
            x == null || x < axisx.min || x > axisx.max || y < axisy.min ||
            y > axisy.max
          ) {
            continue;
          }
    ...
    function drawBar(
      ...
      // clip
      if (
        right < axisx.min || left > axisx.max ||
